...
u32 ov9760_otp_trans(const u8 *ov9760_data_ptr, const u32 ov9760_size, u8 *otp_data_ptr, u32 *otp_size)
{
	...
	OTP_LOG("\noffset--module info:%x lsc1:%x lsc2:%x awb1:%x awb2:%x\n\n", ov9760_data_group_pos.module_info,
...
static u32 check_all_calibration_data(const u8 *ov9760_data_ptr, u32 offset, u32 len)
{
	...
	for(i=0,sum=0;i<tmp_len;i++)  {
        ...
        sum = sum&0xffff;
	OTP_LOG("sum:%x sum_read1:%x sum_read2:%x\n", sum, ov9760_data_ptr[offset+len-2], ov9760_data_ptr[offset+len-3]);
	if(sum != ((((ov9760_data_ptr[offset+len-2]&0x00ff)<<8)&0xff00)|(ov9760_data_ptr[offset+len-3]&0x00ff))) {
